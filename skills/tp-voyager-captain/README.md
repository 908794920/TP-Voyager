...
Codex v1.1.0 的规范入口是唯一 `TP-Voyager` 插件中的 `$tp-voyager:captain`。仓库根部 `SKILL.md` 只保留 legacy migration shim；完整行为规则以 `integrations/codex/local-marketplace/plugins/tp-voyager/skills/captain/SKILL.md` 为准。
