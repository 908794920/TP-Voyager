...
The runtime-side `tp-voyager.manifest.json` remains the canonical MCP launch
and Captain-tool allow-list contract. This Codex plugin intentionally does not
embed a second MCP manifest, startup command, or control surface.

## Quickstart

Use only the TP-Voyager Captain tools mounted by the existing `tp_voyager` MCP.
In normal work, inspect voyage and Crew/model facts, explicitly choose
Crew/model/supported effort, dispatch the bounded task, and consume the bounded
result. Diagnostic tools are not part of the normal Captain contract.

Before delegating, verify that the host-discovered default Captain surface is
the expected bounded seven-tool contract. If required tools are incomplete or
unavailable, fail closed rather than guessing or invoking a vendor CLI.
...
Captain chooses model as well as Crew; Runtime policy validates that explicit choice but does not make it.
...
The default Captain surface is exactly seven tools in v1.0.9.2: the original six control/query tools plus the read-only `render_voyager_panel`. The render tool must use one explicit selector: exact `task_id` for a single Task, or exact `presentation_group_id` / exact `task_ids` for an intentional concurrent view. It must never dispatch/mutate and must never expose prompts, secrets, raw tool output, or hidden/private chain-of-thought.
...
This section is the single Captain-side definition of TP-Voyager panel behavior in Codex. It replaces the separate observability Skill; the existing `tp_voyager` MCP server remains the only runtime/control boundary.

1. `task_dispatch` creates one independent durable Task and its returned `task_id` is the unique identity of that Task. For intentionally concurrent dispatches, the Captain may provide the same explicit `presentation_group_id` to each dispatch; this groups presentation only and never merges lifecycle or result truth.
2. A panel refresh may call only the read-only `render_voyager_panel` tool. For one Task use the exact `task_id`. For an intentional concurrent view use the exact `presentation_group_id` or exact `task_ids`. Never infer members from recent tasks, global tasks, or fuzzy correlation metadata.
3. `task_result(task_id)` is the canonical terminal result truth for the Captain workflow. The panel is a read-only result projection and trace surface, never a second Task state machine or terminal-result authority.
4. Never re-dispatch, resume, cancel, change Crew/model, expand scope, or mutate Runtime state merely to create, refresh, restore, or repair an MCP Apps card. A card reopening first synchronizes through `render_voyager_panel`.
5. The panel may expose only bounded safe projection fields. Never expose a prompt, system text, secret, raw tool output, absolute host path, or hidden/private reasoning. Provider-supplied visible summaries are allowed only when the Runtime has explicitly classified them safe for observation.
6. Machine protocol trailers such as the CrewOutcome envelope are internal transport/projection inputs. Human UI should show the parsed conclusion/evidence/risks/next steps, not the raw protocol payload.
7. The human surface is result-first: canonical full final answer on completion; activity/timeline/files/usage remain independently bounded and collapsible so high-frequency tool events cannot truncate the terminal answer.

Codex plugin packaging gives this Skill the fully qualified name `$tp-voyager:captain` (`tp-voyager` plugin namespace + `captain` Skill name). After installing or updating the plugin, validate it from a **new conversation/session**.
